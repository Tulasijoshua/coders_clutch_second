        ...
        <section className='max-container 2xl:w-[85%] md:w-[95%] w-[90%] mx-auto'>
            ...
            <div className='w-full py-8 flex flex-col space-y-16'>
                <div className=' w-full overflow-auto'>
                    <div className='w-full flex justify-between items-center gap-5 flex-nowrap border-b-2 border-gray-400'>
                        {powerhouse_techs.map(data => (
                            <button 
                                key={data.id}
                                onClick={() => setSelectedTech(data.id)}
                                className={`px-8 flex flex-col pb-3 text-base font-semibold text-center ${selectedTech === data.id ? "border-b-[3px] border-green-700 text-green-700" : "text-gray-600"}`}
                            >
